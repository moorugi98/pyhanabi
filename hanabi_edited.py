...
class SelfIntentionalPlayer(Player):
    ...
    def get_action(self, nr, hands, knowledge, trash, played, board, valid_actions, hints):
        ...
        if self.gothint:  # if I am given a hint about my hands
            ...
            if act.type == HINT_COLOR:
                for ci, card in enumerate(knowledge[nr]):
                    ...
                    # TODO: heuristically favour giving keep hints for older cards
        ...
        #                 pointed = cnt > 0
        #                 if pointed:
        #                     print('pointed')  # TODO: pointed but somehow not keeping
        #                     for col in ALL_COLORS:
        #                         for rank in range(5):
        #                             if card[col][rank] == 1:
        #                                 self.keeplist.add(ci)
        ...
        ### 1D) TODO: heuristically favour giving keep hints for older cards
        #############################################################################
        ###############################################################################
        ...
        if not result:  # no action from direct hints in stage 1
            for i, card in enumerate(possible):  # for each card in hands
                if playable(card, board):
                    ...
                    shift_index(i)  # shift so that keeplist makes sense
        ...
        shift_index(0)  # the 0-th (oldest) will be discarded
